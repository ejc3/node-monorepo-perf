...
Drive the rollout with bun. bun runs the entire rollout natively (frozen-lockfile determinism,
named-catalog version channels, the `workspace:` link for co-development, and the concrete-range publish
rewrite) and against pnpm it wins
...
**Warm install.** When the store and `node_modules` are both warm the gap narrows, and at the top of the
range pnpm-hoisted wins the bun-vs-pnpm install comparison; bun's advantage is specifically the cold path
([TOOLING.md](TOOLING.md#install-bun-vs-pnpm-vs-yarn-4)).
...
(five-way table in [TOOLING.md](TOOLING.md#the-ci-runner-install-frozen-in-a-fresh-container)). (yarn 4, measured in `install-bench.json`'s warm-store dataset, beats pnpm's cold install at
...
(`bench/install-bench.json`, table in [TOOLING.md](TOOLING.md#install-bun-vs-pnpm-vs-yarn-4)); its rollout mechanics are vetted
in [yarn as a Driver](#yarn-as-a-driver) below.)
...
links the local package: the same HEAD-tracking lane bun offers, and the same hazard pnpm's stricter
...
value, ruling out a hazard both bun and yarn allow. Neither blocks the
...
strictness is a real, if minor, point for pnpm. A full safety vet finds two further bun gaps, one more pnpm
safety edge, and otherwise parity ([Adoption Safety](#adoption-safety)). Pick pnpm only if you
...
host-verified (an absent package returns 404 from the CodeArtifact host, not 401 or an npmjs fall-through).

## The Determinism Boundary

Non-reproducible resolution comes from **resolving live**: a non-frozen install, or no committed lockfile.
With a committed lockfile and a frozen install, a `^` or `*` range is inert: the installer reads the exact
pinned version and never consults the registry.
...
Frozen vs not-frozen decides how internal versions move:
...
happens only when there is no usable lockfile.
...
exits 1 and leaves `bun.lock` unchanged, so every call site is frozen by default with no flag to remember. Add
one redundant CI check so a frozen install can never silently re-resolve (`oven-sh/bun#24223`):
...
### 2. Route Cohorts with Named Catalogs
...
A promote is one line: point `stable` at the new version. The central-vs-per-manifest cost is also
...
### 3. Track HEAD with a `workspace:` Cohort
...
### 4. Publishing Bakes a Concrete Range
...
bun reads catalogs from `package.json`, not `pnpm-workspace.yaml` (measured,
...
([WORKSPACE-VS-SEMVER.md §1](WORKSPACE-VS-SEMVER.md#1-the-gate-link-workspace-packages) and
[§2](WORKSPACE-VS-SEMVER.md#2-workspacerange), `scripts/registry-resolution-demo.sh`). They are a **partition**, not
...
from the registry ([WORKSPACE-VS-SEMVER.md §1](WORKSPACE-VS-SEMVER.md#1-the-gate-link-workspace-packages)).
...
edge baked inside every dependent lib's tarball, so holding a cohort on the old core means **republishing the
dependent libs**. The "wave = one catalog line" story holds for a lib consumed **only directly**,
or for a **non-breaking** advance of a universal one. Two coexisting majors of a directly-imported lib is
the diamond behavior ([WORKSPACE-VS-SEMVER.md §3](WORKSPACE-VS-SEMVER.md#3-diamond-resolution-under-semver)), a registry-version story, not a `workspace:` one.
...
- The gate is typecheck-only: it runs no builds or tests. "Run the new lib against all 4,000 apps"
...
protocol, the publish rewrite. Two parts are genuinely N manifest edits: assigning apps to a cohort
(switching a consumer's `catalog:stable` to `catalog:next`), and the *migrate* step of
expand/migrate/contract (rewriting call sites from the old API to the new). Both are codemod territory
...
- **Never delete a published version any lockfile may pin:** deletion breaks every frozen install that
  ...
  makes this a real hazard. Keep at least N-1, and both coexisting majors, published.
...
- **Publish interdependent libs in topological order** (sinks first). A core-lib advance fans out to
  ...
  lacks the change; the [artifact gate](#gating-the-artifact) above is what catches that case.
...
- **pnpm rejects a `workspace:` spec as a catalog value** (measured, `workspaceInCatalog.pnpm`:
  ...
  `workspace:^1.0.0`); bun and yarn both accept it. pnpm's stricter validation rules out one hazard
...
(two bun gaps, one pnpm edge); see [Adoption Safety](#adoption-safety). None blocks the rollout on bun.
