...
- **The fleet shape.** `--preset fleet` regenerates a measured production fleet — 30,000 ~30-file apps / 460 libs, an apps-only universal tier, a semi-universal popular tier, 40% sink libs, depth-15 chains — with all 15 recomputed graph metrics inside tolerance; the core operations and the gate bench run at a real fleet's shape (`optimal-gate-bench.mjs fleet`). ([FLEET.md](FLEET.md), `bench/fleet-shape.json`)
