...
| **Remote cache (CI economics)** | every CI runner starts cold, so a centralized Turborepo cache turns the O(repo) cold start into a restore for the second-and-later runner: whole-repo typecheck **23.6s → 1.9s** (12.5×, 300:100) and **67.2s → 5.9s** (11.4×, 1,000:200); a build **62.7s → 4.0s** (15.5×). Restore is itself O(repo) (the warm-cache graph-load floor), so the speedup holds ~11–12× as the repo grows rather than widening; measured over a localhost cache, so a real network adds the download (≤0.5 MB typecheck, 247 MB build). Across a 10-runner same-closure fleet it amortizes ~5.6×; it restores **486 of 500** tasks after a leaf edit but **0** after a foundation edit. | [LIMITS](LIMITS.md) |
