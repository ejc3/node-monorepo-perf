...
- **yarn 4 at fleet scale.** PnP installs the 30,460-package workspace truly-cold in **41.3s** (one 62MB `.pnp.cjs`; the node-modules farm it replaces is 4.64M entries and 436.4s), and a native-PnP tsgo build runs the whole-program gate through it in **59.2s** vs the node-modules gate's 60.7s (different dev builds — the same-binary control is tsgo-pnp-bench's 20:10 matrix, identical under both linkers). Stock tsgo still fails PnP (30,000 unresolved). ([TOOLING.md](TOOLING.md#yarn-pnp-toolchain-compatibility), `bench/yarn-fleet-bench.json`)
