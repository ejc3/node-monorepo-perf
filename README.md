...
| **yarn as rollout driver + PnP boundary** | yarn 4 runs every rollout mechanic natively — byte-identical lockfile resolves, `--immutable` fail-closed, **CI auto-immutable with zero config** (the determinism default bun only gets from a committed `bunfig` line), named catalogs in `.yarnrc.yml` with 0-manifest repoints, `workspace:` catalog lanes, and `yarn pack` baking concrete ranges for both `workspace:^` and `catalog:`. Its PnP linker's compat cost is priced: tsc/turbo/oxlint run under PnP; tsgo and `next build` do not (node-modules control passes all five). | [ROLLOUT](ROLLOUT.md), [TOOLING](TOOLING.md) |
...
| **Type-checking** | whole-repo typecheck is O(repo): cold 19s → 233s, warm cache 1.5s → 20.5s — warm is still O(repo) because Turbo hashes every package on a full hit. tsgo (TypeScript's native port) is ~12× faster per check, drop-in for modern configs, still beta (it drops `node10`/legacy targets and has no LSP plugin API yet). At a million modules tsgo checks in 68.6s at 53.7GB RSS, near-linear across the 100× range, and a failing check costs the same as a passing one; Flow closes to within 25% of the batch check by 500k (at a third of tsgo's memory there) and its live server answers one edit in under a second through 250k, but its 0.321 server wedged on a 500k recheck (a WorkerCanceled panic recorded in the canonical dataset; fixed upstream, unreleased — facebook/flow#9454), so tsgo holds the only measured 1M column. The mechanic that makes 1M interactive is tsgo's own daemon: `tsgo --lsp` opens the million-file program in 17.4s and serves a 2.2s asserted squiggle at 59.7GB RSS (tsserver needs 25.7s to open 100k), with one boundary — its completion enumerates the full symbol space and times out at 500k+ (tsserver's bounded 1,067-entry set: 19–20ms). | [TYPECHECKERS](TYPECHECKERS.md) |
