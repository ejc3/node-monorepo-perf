...
- **The fleet shape.** `--preset fleet` regenerates a measured production fleet — 30,000 ~30-file apps / 460 libs, an apps-only universal tier, a semi-universal popular tier, 40% sink libs, depth-15 chains — with all 15 recomputed graph metrics inside tolerance. Measured at full scale (~1.03M generated files): one tsgo program type-checks the whole workspace from source in **60.7s** (**10.1×** faster than the per-package turbo pipeline's 613.0s / 30,708 tasks — not like-for-like: the pipeline also builds each lib's dist) and catches a breaking foundation rev with all 30,000 apps red in **59.5s**; on a 192-core box the one-program check does **not** get faster (65.8s) while the pipeline improves 1.6×. `make typecheck-whole` packages the one-command gate; slicing the same check into K concurrent per-subset programs cuts it to **9.9s** on 64 cores and **6.3s** on 192 (union-verified identical verdict — the box the one-program gate wastes becomes the fastest way to run it) ([FLEET.md](FLEET.md#the-sliced-gate-using-the-whole-box), `bench/sliced-gate-bench.json`). ([FLEET.md](FLEET.md), `bench/fleet-gate-bench.json`, `bench/fleet-gate-bench.pbox.json`)
- **Flow on the fleet shape.** A Flow-dialect mirror of the fleet (876,440 files, the same workspace import graph — both checkers flag exactly 30,171 call sites on the breaking rev — though a smaller ambient surface than the TS program): Flow's batch check is 1.3× slower than tsgo (79.5s vs 60.7s) at **2.5× less memory** (20.1GB), and its resident server answers the full 30,000-apps-red breaking verdict **incrementally in 14.9s** (45ms when nothing changed) — a resident mechanic tsgo does not have (its `--watch` re-checks in ~22s at 1M files). ([TYPECHECKERS.md](TYPECHECKERS.md#flow-on-the-fleet-shape), `bench/fleet-flow-bench.json`, `bench/fleet-flow-bench.pbox.json`)
- **yarn 4 at fleet scale.** PnP installs the 30,460-package workspace truly-cold in **40.8s** (one 62MB `.pnp.cjs`; the node-modules farm it replaces is 4.64M entries, 441.5s, and CoW-dependent), and a native-PnP tsgo build runs the whole-program gate through it in **60.1s vs 58.9s for the same binary over node-modules** — PnP costs ~2% on the check. Stock tsgo still fails PnP (30,000 × TS2503). ([TOOLING.md](TOOLING.md#yarn-pnp-toolchain-compatibility), `bench/yarn-fleet-bench.json`)
