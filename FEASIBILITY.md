...
Paid once per change, not per person. The resolve (98–99% of a cold install,
`lockfile-bench.json`) is committed to `pnpm-lock.yaml`; everyone else runs `pnpm install
--frozen-lockfile` and skips it. A build task runs once anywhere; others download the
output — but build-once amortization **requires the remote cache on**
...
(rebuilds ~90% of the repo). Lockfile conflicts auto-resolve (253 markers → 0); catalogs
change 0 manifests vs 25 pinned (`lockfile-merge-bench.json`).
