...
pay their cost for no benefit, so a polyrepo or separate installs fits better. The
...
| cold typecheck | a cache-busting change | Remote Cache: a task runs once anywhere, others download the output (`FULL TURBO`) ([LIMITS.md](LIMITS.md#remote-cache-amortizing-the-orepo-cold-start)); `--affected` only considers changed packages |
| graph-load | every `turbo` command | inherent (compute the closure); Vercel reports an 11× cut, ~1,000 pkgs 8.1s→0.7s ([Vercel's graph-load work](https://vercel.com/blog/making-turborepo-ninety-six-percent-faster-with-agents-sandboxes-and-humans)) |
| lockfile churn / conflicts | a version change | catalogs (0 manifest edits vs 25 pinned, `lockfile-merge-bench.json`); `pnpm install` auto-resolves conflicts, 253 markers → 0 ([pnpm's git docs](https://pnpm.io/git)) |
...
lockfile is first created; a later dependency change re-resolves only the delta
...
## Triggers for the Slow Operations

Each O(repo) cost is dormant until a specific trigger fires it:

- **The full from-scratch resolve runs only when there is no usable
lockfile**: the first install, a deleted or corrupt lockfile, or `--no-lockfile`.
It is **not** triggered by a fresh clone, a cold CI runner, or a deleted
...
only the no-lockfile case pays the full cold resolve (cost-model table above). The
exception is a **catalog bump** (51.3s): it re-resolves that shared dep across
every importer, cheap in *edits* (0 manifests) but not in *time*.
- **Cold typecheck** recurs when a change invalidates many packages' cache: a shared
...
- **Graph-load** is paid on every `turbo` command (before it computes the closure):
sub-second to low-seconds, and being reduced (see the Vercel link above).
- **Lockfile merge conflicts** arise when two branches change dependencies in
overlapping lockfile regions; `pnpm install` auto-resolves them (amortizer table
above).
- **Version skew** accrues while apps sit off the catalog version. Turborepo hashes a
package's resolved dependency versions as part of its task inputs, so an
off-catalog version produces a different input hash and a cache miss for that
package, which is the documented reason catalogs keep cache hashes stable
([OPTIMIZATIONS.md §2.3](OPTIMIZATIONS.md#23-caching-local-then-remote)).
`lockfile-merge-bench.json` measures the manifest/lockfile
churn of skew (25 changed manifests), not the cache-hit rate.

## Graph-Load Cost for Single-App Work
...
## The Package-Manager Lever: pnpm vs bun

Turborepo supports pnpm, npm, yarn, and bun, all at its Stable tier
([support policy](https://turborepo.dev/docs/support-policy)). The resolve
dominates pnpm's install; on a full re-resolve, bun is ~62–357× faster than pnpm
across 200–2,000 apps (`install-bench.json`). yarn 4 is measured in the same
dataset and also beats pnpm at every scale, fastest of all at 2,000 apps, though
PnP cannot run this repo's tsgo/`next build` stack (`pnp-compat-bench.json`).
Per-cell numbers are in [TOOLING.md](TOOLING.md#install-bun-vs-pnpm-vs-yarn-4).
...
for workspaces, [isolated-installs docs](https://bun.com/docs/pm/isolated-installs)),
catalogs and the `workspace:` protocol
([workspaces docs](https://bun.com/docs/pm/workspaces)) with
pnpm-lockfile migration ([lockfile docs](https://bun.com/docs/pm/lockfile)). The
...
of ext4's hardlinks, with equal relink time (2.9s vs 3.1s) and ~0 extra disk (0.4 MB
exclusive of 338 MB), with CoW-independent inodes (`fs-bench.json`).

## Centralized Shared Versions plus Independently Published Packages

This setup is a hybrid, and the two halves are orthogonal:
...
registry), [WORKSPACE-VS-SEMVER.md §7](WORKSPACE-VS-SEMVER.md#7-per-app-workspaces)
materializes it live
...
| capability this setup needs | pnpm 10 | bun 1.3 |
|---|---|---|
| catalogs (centralize shared) | yes (`pnpm-workspace.yaml`) | yes ([workspaces docs](https://bun.com/docs/pm/workspaces)); isolated+catalog had bugs in 1.3 ([#23615](https://github.com/oven-sh/bun/issues/23615)) |
| `workspace:` + publish-rewrite to npm | yes | yes: `bun pm pack` bakes `workspace:^`→`^2.5.0` (measured, `bench/wave-rollout-bench.json`) |
| semver-internal resolves from registry | yes (`link-workspace-packages=false`) | `workspace:` → local; plain-semver local-vs-registry behavior *(unverified here)* |
| isolated linker (no phantom deps) | yes (default) | yes (default for workspaces, [isolated-installs docs](https://bun.com/docs/pm/isolated-installs)) |
| lockfile merge conflicts auto-resolved | yes, measured (amortizer table above) | `bun.lock` ([lockfile docs](https://bun.com/docs/pm/lockfile)) *(auto-resolve not benchmarked here)* |
| whole-workspace cold install at scale | slow ([cost model](#the-cost-model)); resolve once, frozen elsewhere | 62–357× faster install (measured) |
...
cold resolve ([cost model](#the-cost-model)) and the inode-heavy linker.
...
2,000 apps ([TOOLING.md](TOOLING.md#install-bun-vs-pnpm-vs-yarn-4)); its rollout
...
[ROLLOUT.md](ROLLOUT.md#yarn-as-a-driver)). The PnP-linker install wins
carry a toolchain boundary: tsgo and `next build` do not run under PnP on this
repo's stack ([TOOLING.md](TOOLING.md#yarn-pnp-toolchain-compatibility)); the yarn
node-modules linker has no such caveat.

Buck2 and Bazel are build systems, not package managers; they would replace
Turborepo's task orchestration, not pnpm, and are a separate comparison.
...
miss until it returns to the catalog (the hashing mechanism,
[OPTIMIZATIONS.md §2.3](OPTIMIZATIONS.md#23-caching-local-then-remote); the
cache-hit rate itself is not benchmarked here). The
...
| same, but install/resolve time dominates | same, with bun (accepting the 1.3 strict-mode caveats) or yarn 4 for installs ([TOOLING.md](TOOLING.md#install-bun-vs-pnpm-vs-yarn-4)) |
...
- **4,000 apps / 4,300 packages (measured):** the cold install and cold typecheck
reach the cost-model table's maxima, bearable only if remote cache + prune keep
them rare. The isolated
...
entries (21,914 vs 50,159 at 2,000 apps, `install-bench.json`), and yarn PnP removes
...
custom, [Vercel limits](https://vercel.com/docs/limits)), a deployment-side ceiling
