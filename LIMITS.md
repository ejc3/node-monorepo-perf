...
7. **Deploy-platform per-project model.** One Vercel project per deployable app = 20k projects. A push to a foundation lib evaluates "affected" across up to 20k projects (turbo-ignore gates the *build*, not the webhook/queue fan-out); org build-concurrency caps apply.
...
1. Lockfile at 10k/20k — lines, MB, and pnpm parse time per install.
2. Full-tree inode count (not just root `node_modules`) for isolated vs hoisted vs `pnp`.
