...
**yarn 4 at fleet scale** (`scripts/yarn-fleet-bench.mjs`, 30,000 apps / 460 libs, the [FLEET.md](FLEET.md) shape with the fleet gate's exact devDependency set; `bench/yarn-fleet-bench.json`): PnP installs the workspace **truly cold in 41.3s** (no lockfile, fresh global cache, network) and 38.6s warm — one 62MB `.pnp.cjs` instead of the node-modules farm. yarn's own node-modules linker takes **436.4s** on the same tree (4,641,433 `node_modules` entries); the fleet gate's bun warm-store install is 180.6s against the same workload (a friendlier state than PnP's truly-cold row, and PnP still wins 4.4×). The type gate closes the loop: the native-PnP tsgo build (head of the PR line consolidated in [microsoft/typescript-go#1966](https://github.com/microsoft/typescript-go/pull/1966); binary sha + git sha recorded) runs the whole-program fleet gate **through `.pnp.cjs` in 59.2s / 50.9GB**, against the node-modules gate's 60.7s / 51.3GB (`bench/fleet-gate-bench.json`; that run used the stock pin, a different dev build — the same-binary PnP-vs-node-modules control is tsgo-pnp-bench's 20:10 matrix, where the patched build scores identically under both linkers) — and catches the breaking foundation rev with all 30,000 apps red in 60.7s. Stock tsgo on the same PnP tree fails with 30,000 unresolved-name/module errors (TS2503/TS2307-class), the pnp-compat boundary at full scale. PnP's install win costs nothing on the check once the resolver exists; what still stands between this stack and PnP is Turbopack (rspack/webpack build, above) and the patch not yet being shipped.
