...
Measured by `scripts/install-bench.mjs`. Environment in `bench/env.json` (Neoverse-V1, 64 cores, 135 GB). Each manager runs at its own workspace default plus, for pnpm and yarn, the alternate linker: pnpm-isolated (pnpm's default) and pnpm-hoisted (flat); bun's workspace default, which since bun 1.3 is the isolated layout (a `node_modules/.bun` store; its entry counts land near pnpm-isolated's rather than the flat layouts'); yarn 4.17.0 under `node-modules` (flat, the layout match for pnpm-hoisted) and under PnP (yarn's default: no `node_modules` at all, a `.pnp.cjs` resolution table over global-cache zips, with only native packages materialized under `.yarn/unplugged`).
...
Reading it:
- pnpm cold install is ~linear in package count (47.8s → 471.2s, 10× apps); bun has a far smaller constant (0.13s → 7.5s): roughly 357× faster cold than pnpm's default isolated at 200/100, ~103× at 1,000, ~62× at 2,000 (pnpm-hoisted is within ~3% of isolated, so its ratios track). The gap is not a cache effect: truly-cold (fresh store + metadata, real network) bun stays faster (1.2s vs 24.0s in one sample), though that path is network-bound; see the note above.
...
- Warm relink (lockfile present) is where the linker shows up: pnpm-hoisted relinks in 4.7s at 2,000 vs pnpm-isolated's 15.2s; recreating the isolated symlink farm is the warm-relink cost. bun's warm (9.5s) lands above its cold (7.5s) at 2,000. yarn-PnP's warm is the fastest at 1,000 and 2,000 apps (2.1s, 2.9s): with no `node_modules` to materialize, warm is mostly rewriting `.pnp.cjs`.
...
The PnP compatibility cost stated qualitatively above, measured by `scripts/pnp-compat-bench.mjs`
on this repo's own stack: one generated workspace (20 apps / 10 libs) installed twice by the
same pinned yarn (PnP and node-modules, the control), with every tool run through yarn in both
trees. A tool that fails both trees is a scaffold problem (the bench hard-fails); a tool that
passes the control and fails under PnP is a PnP incompatibility (`bench/pnp-compat-bench.json`).
...
### Closing the Gap: Native PnP for tsgo, and Next Under PnP
...
measured by `scripts/tsgo-pnp-bench.mjs` on a workspace installed at Yarn's PnP defaults
(the manifest inlined in `.pnp.cjs`, no `.pnp.data.json` sidecar) with the node-modules
linker as the control
...
### The Fast Bundler Under PnP: rspack
...
PnP "got merged in rspack in less than a week." `scripts/rspack-pnp-bench.mjs` builds the same
Next App Router app (next + react +
...
Which bundler ran is proven: a build counts only with a populated `.next`
...
Rust) emits none of them and only rides Next's outer `run-webpack` wrapper, and Turbopack emits
...
### Build Speed: Turbopack vs rspack vs webpack

The compat matrix builds a one-page app, where build time is framework startup, not bundling. This
prices real build speed: `scripts/rspack-turbopack-speed-bench.mjs` generates a 60-route App Router
app (each route importing shared
...
median of three (`bench/rspack-turbopack-speed-bench.json`;
...
speed, not app difference. node-modules + Turbopack is the fastest configuration; a PnP shop
gives up that lead, and rspack is its fast option, a bit ahead of webpack cold and even on warm.

### Specifier Form and node-linker

Does the `workspace:` spec form or the linker mode change install perf? Measured by `scripts/perf-matrix.mjs`; cold install at 300/100:
...
Measured by `scripts/container-install-bench.mjs`. What a real CI runner pays: a checkout carrying
the **committed lockfile**, installed frozen (`pnpm --frozen-lockfile`, `bun --frozen-lockfile`,
`yarn --immutable`, `npm ci`), measured in true isolation. Every sample runs in a fresh
rootless-podman container (hermetic env, digest-pinned node image with the pinned toolchain baked
in), at 1,000 apps / 200 libs, median of five samples in a round-rotated tool order (every tool
takes every position once). Two variants: **fresh runner** =
...
- Spreads are tight (bun 890–900ms; pnpm 8.7–9.0s across five network-live samples), and the samples
...
Measured by `scripts/build-bench.mjs`. Full `turbo run build` of 40 apps + 24 libs, concurrency 12, 64-core machine. Next is App Router (SSR/RSC); Vite is a client SPA, so this compares build tooling and output, not equivalent features.

| framework | build (all 40 apps) | CPU | peak RSS | total output |
|---|---|---|---|---|
| Next (App Router) | 17.2s | 2,798% (~28 cores) | 741 MB | 156.8 MB (`.next`) |
| Vite (SPA) | 7.6s | 1,187% (~12 cores) | 193 MB | 7.7 MB (`dist`) |
...
[its license](https://voidzero.dev/posts/voidzero-cloudflare)); v0.2.2 (beta) is measured here,
by `scripts/vite-task-bench.mjs` and `scripts/vite-plus-tools-bench.mjs`.
...
### Task Orchestration: Vite Task vs Turborepo

The two runners execute the identical dep-free `typecheck:tsgo` task set (identical
generated DAG, tsgo binary, and pnpm-installed tree; concurrency pinned to 64 on both,
cold/warm asserted from each runner's own summary; `bench/vite-task-bench.json`). The
runners differ in mechanism: turbo hashes **declared inputs** and respects `.gitignore`
(its rungs run under the source-visibility workaround); Vite Task **fs-traces what each
task reads** (LD_PRELOAD/seccomp), and it cached the fully gitignored generated tree with
zero config (probed with `git check-ignore`, all-cached warm asserted).
...
Whole-repo typecheck, turbo wins by 2–3.7×: vp pays fs-tracing and
...
warm grows with the repo (1.2s → 3.0s, the graph-load + hashing floor [LIMITS.md](LIMITS.md)
...
1 of 1,200 (only the edited lib, its 558 dependents left stale). The dep-free task shape
both runners compare on gives turbo's package-scoped input hashing no cross-package edge
to propagate through; turbo's mechanisms for that are `dependsOn: ["^build"]` (propagates
through the build chain, at the price of scheduling builds) or `globalDependencies`
(invalidates everything), and per-dependent propagation without one of those is not
expressible.

**The tracer's boundary: self-mutating tasks.** vp refuses to cache any task that
writes a file it also read. `next build` cannot be cached under vp because it rewrites
its own inputs (recorded:
...
### The Tool Layer: `vp check` and `vp build`

**`vp check --no-fmt`** (oxlint + tsgolint type errors in one pass) vs the same pinned
engines run separately, on a 920-file source-only corpus (`bench/vite-plus-tools-bench.json`;
positive control: all engines
...
On this repo's stack the Vite+ integration layer costs time at every measured point
except the focused loop (cold and warm, both scales) and small-read-set tasks, where Vite
Task's O(closure) behavior beats turbo's O(repo) warm floor. Its fs-traced cache is the
first measured runner that is correct on gitignored source with zero config and correct
on cross-package edits with zero task config. The price is refusing self-mutating tasks
