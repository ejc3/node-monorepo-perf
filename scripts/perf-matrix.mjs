...
const APPS = opt("apps", "1000"), LIBS = opt("libs", "200"), MODULES = opt("modules", "16");
...
const sh = (cmd) => execSync(cmd, { cwd: ROOT, stdio: ["ignore", "pipe", "pipe"], maxBuffer: 1 << 30, env }).toString();
const num = (c) => { try { return parseInt(sh(c).trim() || "0", 10); } catch { return 0; } };

const results = [];
for (const v of VARIANTS) {
  console.log(`\n=== ${v.label} (${v.note}) @ ${APPS} apps / ${LIBS} libs ===`);
  sh(`node scripts/generate.mjs --apps ${APPS} --libs ${LIBS} --modules ${MODULES} ${v.versioned ? "--versioned" : ""} --clean`);
  ...
  const lock = existsSync(join(ROOT, "pnpm-lock.yaml")) ? readFileSync(join(ROOT, "pnpm-lock.yaml")) : Buffer.from("");
  const rec = {
    label: v.label, note: v.note, apps: +APPS, libs: +LIBS, linker: v.linker, versioned: v.versioned,
    installMs,
  ...
  console.log(`  install ${installMs}ms · lockfile ${rec.lockfileLines} lines · nm ${rec.nmEntries} entries (${rec.nmSymlinks} symlinks)`);
  results.push(rec);
}
...
for (const r of results) {
  console.log(`${r.label.padEnd(10)} install ${String(r.installMs).padStart(7)}ms (${fmtPct(pct(r.installMs, base.installMs))})  nm ${String(r.nmEntries).padStart(8)} (${fmtPct(pct(r.nmEntries, base.nmEntries))})`);
}
