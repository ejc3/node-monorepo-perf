...
const TSCONFIG = JSON.stringify({
  compilerOptions: {
    ...
    declaration: true, outDir: "dist", rootDir: "src", strict: true, skipLibCheck: true
  },
  include: ["src"]
}, null, 2);
...
pkg("registry/widget-v1", base("@ejc3/widget", "1.0.0"),
...
pkg("registry/widget-v2", base("@ejc3/widget", "2.0.0"),
...
pkg("registry/alpha", base("@ejc3/alpha", "1.0.0", { "@ejc3/widget": "^1.0.0" }),
...
pkg("registry/beta", base("@ejc3/beta", "1.0.0", { "@ejc3/widget": "^2.0.0" }),
...
writeFileSync(join(consumerDir, "package.json"), JSON.stringify({
  name: "@ejc3/diamond-consumer", version: "0.0.0", private: true, type: "module",
  scripts: { start: "node run.mjs" },
  dependencies: { "@ejc3/alpha": "^1.0.0", "@ejc3/beta": "^1.0.0" }
}, null, 2) + "\n");
writeFileSync(join(consumerDir, "run.mjs"),
...
writeFileSync(join(ovr, "pnpm-workspace.yaml"),
`packages:
  - "packages/*"
  - "consumer"
`);
writeFileSync(join(ovr, "package.json"), JSON.stringify({
  name: "diamond-override-root", version: "0.0.0", private: true,
  // Flip the shared lib to the LOCAL workspace copy for everyone — collapses the diamond.
  pnpm: { overrides: { "@ejc3/widget": "workspace:*" } }
}, null, 2) + "\n");
writeFileSync(join(ovr, ".npmrc"),
`# semver internal deps resolve from the registry by default (pnpm 8+).
link-workspace-packages=false
`);

console.log(JSON.stringify({ scaffolded: "examples/diamond", packages: ["@ejc3/widget@1.0.0", "@ejc3/widget@2.0.0", "@ejc3/alpha@1.0.0", "@ejc3/beta@1.0.0", "@ejc3/diamond-consumer"] }));
