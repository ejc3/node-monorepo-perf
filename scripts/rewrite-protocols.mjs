  ...
  for (const field of ["dependencies", "devDependencies", "peerDependencies", "optionalDependencies"]) {
    ...
    for (const [name, spec] of Object.entries(deps)) {
      ...
      if (spec === "catalog:" || spec.startsWith("catalog:")) {
        if (catalog[name]) { deps[name] = catalog[name]; touched = true; }
        else throw new Error(`no catalog entry for ${name} (in ${file})`);
      }
    }
  }
  if (UNPRIVATE && pkg.private) { delete pkg.private; touched = true; }
  if (touched) { writeFileSync(file, JSON.stringify(pkg, null, 2) + "\n"); changed++; }
}
console.log(JSON.stringify({ dir: DIR, catalogKeys: Object.keys(catalog), packagesRewritten: changed, unprivate: UNPRIVATE }));
