...
function entries() {
  ...
  const r = spawnSync("bash", ["-c", "find . -path '*/node_modules/*' -printf '.' 2>/dev/null | wc -c"], { cwd: DIR, encoding: "utf8", maxBuffer: 1 << 28 });
...
const rmNM = () => spawnSync("bash", ["-c", "find . -name node_modules -type d -prune -exec rm -rf {} +"], { cwd: DIR });
...
for (const { apps, libs } of SCALES) {
  ...
  const depEdgesVerified = verifyComplete(); const piNm = entries();
