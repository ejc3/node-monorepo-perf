...
import { rmSync, mkdirSync, writeFileSync, readFileSync, existsSync, openSync, closeSync } from "node:fs";
...
const SCALES = (process.argv[2] || "200:100 1000:200 2000:300").trim().split(/\s+/).map((s) => {
  const [a, l] = s.split(":");
...
function setup(apps, libs) {
  ...
  writeFileSync(join(DIR, "package.json"), JSON.stringify({ name: "lf-bench", private: true, workspaces: ["apps/*", "packages/*"] }) + "\n");
}
const rmAll = () => { rmSync(join(DIR, "node_modules"), { recursive: true, force: true }); for (const f of ["pnpm-lock.yaml", "bun.lock", "bun.lockb"]) rmSync(join(DIR, f), { force: true }); };
...
for (const { apps, libs } of SCALES) {
  ...
  const rec = {
    apps, libs,
    pnpm: { resolveMs: pResolve, verifyMs: pVerify, fullMs: pFull, lockLines: pLock.lines, lockBytes: pLock.bytes, resolveSharePct: Math.round((pResolve / pFull) * 100) },
    bun: { resolveMs: bResolve, fullMs: bFull, lockLines: bLock.lines, lockBytes: bLock.bytes, resolveSharePct: Math.round((bResolve / bFull) * 100) },
  };
  out.push(rec);
  ...
  console.log(`  pnpm  resolve ${pResolve}ms verify ${pVerify}ms full ${pFull}ms (resolve ${rec.pnpm.resolveSharePct}% of full) lock ${pLock.lines} lines`);
  console.log(`  bun   resolve ${bResolve}ms             full ${bFull}ms (resolve ${rec.bun.resolveSharePct}% of full) lock ${bLock.lines} lines`);
}
