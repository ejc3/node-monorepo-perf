...
The equal relink times above are a buffered, page-cache result (a warm `--offline` materialization that writes through RAM), so they say nothing about the raw device. Measured directly (`scripts/fs-iops-bench.mjs`, `bench/fs-iops-bench.json`): the two mounts are different NVMe devices, and at the device layer the btrfs scratch NVMe is faster in every regime. With 4K random I/O at queue depth 16 and `O_DIRECT` (no OS page cache; a 1 GiB working set), the btrfs NVMe (`/dev/nvme0n1`) does **123,628 read / 14,486 write IOPS** at **241µs / 1,581µs p99**, versus the working-tree ext4 NVMe (`/dev/nvme2n1p1`) at **3,498 / 3,144 IOPS** and **5,669µs / 8,290µs p99** — **×35.3 read, ×4.6 write**, with **×23.5 / ×5.2 lower p99**. The gap is widest for durable writes: a per-file `fsync` of 5,000 × 512B files runs at **5,162/s on btrfs vs 327/s on the working tree (×15.8)** — the ext4 device serializes durable writes at ~327/s, a ~3ms barrier per `fsync`, in line with its high random-write latency (8,290µs p99), so fsync-bound work (git objects, sqlite, lockfile flushes) is an order of magnitude slower there.

The one regime where the two are close is exactly the one fs-bench measured: a **buffered** small-file burst is ×1.31 (btrfs 54,536 vs 41,517 files/s, create-only) and the relink is equal within noise — because buffered writes land in the page cache (RAM), so device speed barely shows. The device gap only appears once a workload actually touches the device: a truly-cold install whose reads miss the page cache, or any fsync-heavy step. A `node_modules` materialization is the buffered kind, so it lands in the close regime — but a cold install or fsync-bound measurement on the working-tree volume understates the same workload on the btrfs NVMe, so scratch-heavy benches run there: `real-app-bench`'s `REAL_APP_WORK` defaults to the btrfs mount.
