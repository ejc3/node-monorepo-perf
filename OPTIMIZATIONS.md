...
The equal relink times above are a buffered, page-cache result (a warm `--offline` materialization that writes through RAM), so they say nothing about the raw device. Measured directly (`scripts/fs-iops-bench.mjs`, `bench/fs-iops-bench.json`): the two mounts are different NVMe devices, and at the device layer the btrfs scratch NVMe is faster in every regime. With 4K random I/O at queue depth 16 and `O_DIRECT` (no OS page cache; a 1 GiB working set), the btrfs NVMe (`/dev/nvme0n1`) does **123,628 read / 14,486 write IOPS** at **241µs / 1,581µs p99**, versus the working-tree ext4 NVMe (`/dev/nvme2n1p1`) at **3,498 / 3,144 IOPS** and **5,669µs / 8,290µs p99** — **×35.3 read, ×4.6 write**, with **×23.5 / ×5.2 lower p99**. The widest gap is random reads (×35.3 IOPS, ×23.5 lower p99); among writes it is durable ones that diverge most — a per-file `fsync` of 5,000 × 512B files runs at **5,162/s on btrfs vs 327/s on the working tree (×15.8)**, far above the ×4.6 buffered random-write gap. The ext4 device serializes durable writes at ~327/s, ~3ms per fsync'd file, in line with its high random-write latency (8,290µs p99), so fsync-bound work (git objects, sqlite, lockfile flushes) is an order of magnitude slower there.

The one regime where the two are close is the same buffered, page-cache regime fs-bench's relink runs in: a **buffered** small-file burst is ×1.31 (btrfs 54,536 vs 41,517 files/s, create-only) and the relink is equal within noise — because buffered writes land in the page cache (RAM), so device speed barely shows. The device gap only appears once a workload actually touches the device: cold reads that miss the page cache, or any fsync-heavy step. A `node_modules` materialization is the buffered kind, so it lands in the close regime — but by the measured device gap a cold install (cold reads plus dependency-tree fsyncs) or any fsync-bound step would run slower on the working-tree volume than on the btrfs NVMe, which is why scratch-heavy benches run there: `real-app-bench`'s `REAL_APP_WORK` defaults to the btrfs mount.
