...
Vite+'s task runner was priced against turbo on identical task sets: turbo wins the
whole-repo typecheck by 2–3.7×, while Vite Task wins the focused warm loop (0.86s vs
turbo's 3.0s at 1,000 apps) and stays flat across 3× repo growth. Vite Task cannot cache
`next build`, and `vp check`/`vp build` cost more than the tools they wrap
...
Everything here is O(closure): one app plus the libs it imports. Fresh (first time) vs
subsequent (repeat).
...
This is O(closure) too: one leaf lib plus the libs it imports; its pre-merge gate re-checks
the lib's dependents.
...
reinstall or publish.

### Workspace Author

The worst case in the monorepo, and the O(repo) one: rev the library all 4,000 apps depend
on, and catch a breaking type error before it merges. (`bench/optimal-gate-bench.json`, `bench/typecheck-parity-bench.json`;
...
(4,000 apps / 300 libs.) The cost is all in the cold open: tsgo's native LSP cuts the one part
that scales (project load) by ~19× and uses ~30% less memory, and once warm both servers
resolve cross-package go-to-def to the exact lib source in ≤2ms. Detail in
[LIMITS.md](LIMITS.md#editor-and-language-server).
...
- **Install** of the whole workspace (~21s, warm store): paid on a clean clone or in CI.
  pnpm's no-lockfile cold-resolve (lockfile authoring) is a different operation, 233s at
  1,000:200 (`bench/install-modes-bench.json`); on a full re-resolve bun is ~62–357× faster
  than pnpm (`bench/install-bench.json`). yarn 4 is the fastest cold and warm installer at
  2,000 apps, but PnP cannot run the stock tsgo or Next's default Turbopack build
  (`bench/pnp-compat-bench.json`; both have a green path via a native PnP resolver for
  tsgo and `next build` with webpack or rspack, `bench/tsgo-pnp-bench.json` +
  `bench/rspack-pnp-bench.json`; the node-modules linker has no such boundary). The install figures in this summary were
  measured with bun as the installer; the bun-vs-yarn reconciliation is in
  [OPTIMAL-STACK.md](OPTIMAL-STACK.md#installing-the-workspace), and yarn as a rollout
  driver is vetted in [ROLLOUT.md](ROLLOUT.md#yarn-as-a-driver).
- **A whole-repo dist build** (release-everything) scales with package count.

The whole-repo **build and typecheck** are amortized across a CI fleet by a remote cache (install amortizes separately, through the warm store and committed lockfile above). Every runner starts with an empty local cache, but after the first seeds the shared cache, each later runner *restores* turbo's task outputs instead of recomputing: a whole-repo typecheck drops 23.6s → 1.9s (12.5×) at 300:100 and 67.2s → 5.9s (11.4×) at 1,000:200 (`bench/ci-cache-bench.json`). Over a real network the restore is free for the small typecheck cache and 3.5s→5.8s localhost→cross-region for the 247 MB build cache, still ×10 under cold (`bench/ci-cache-network-bench.json`). The cache helps only the second-and-later consumer of an *unchanged* artifact: a leaf edit lets a fresh runner restore 486 of 500 tasks, a universal-foundation edit 0 of 500. Detail in [LIMITS.md](LIMITS.md#remote-cache-amortizing-the-orepo-cold-start).
...
medians of three, bun install and turbo single runs):
...
real app in means modernizing the config and adding an ambient `*.css` declaration. The adapted
...
errors); taxonomy shows 13: seven cannot-find-module errors (TS2307), the sampled ones for
`contentlayer/generated` codegen this bench doesn't run, and six for genuine dependency drift
...
- **tsgo is a preview build.** It does not emit `dist` (the turbo path uses tsc via `^build`
  ...
  deliberately type-heavy corpus. That is one corpus with `skipLibCheck`, not a general
  parity proof.
...
- **bun is adoptable but not a strict safety superset of pnpm** (`bench/bun-safety-bench.json`,
  bun 1.3.14 vs pnpm 10, head-to-head): two real gaps (bun's built-in allowlist runs some
  registry `postinstall` scripts pnpm 10 blocks; no fail-closed strict-peer knob), one pnpm
  edge (phantom isolation in single-package projects; workspaces are parity), the rest
  parity. Full treatment in [ROLLOUT.md](ROLLOUT.md#adoption-safety).
- The focused-gate **warm** numbers carry turbo's per-invocation graph-load + cache-restore
  cost over the 4,400-package workspace and are noisy run-to-run (reported as medians of
  three): the app and lib warm gates don't order reliably between runs. That floor is
