...
| Reset state / isolate the store between configs | [benchmarking crimes](https://gernot-heiser.org/benchmarking-crimes.html) | isolated `/tmp` workspace; each scale freshly generated; node_modules+lockfile removed per measurement; truly-cold uses a throwaway `--store-dir` + fresh `--config.cache-dir` (pnpm metadata) + `bun pm cache rm` |
