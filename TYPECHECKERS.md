...
`scripts/typecheck-bench.mjs` generates N cross-referencing modules in one program and times `--noEmit` for tsc and for tsgo (the TypeScript native Go port, shipped as `@typescript/native-preview`); each number is the median of five timed runs after a discarded warmup.
...
growing through 10k, 100k, 250k, 500k, and 1,000,000 modules? `scripts/tsgo-scale-bench.mjs`
sweeps that range for
three checkers: **tsgo** (the subject), **tsc** (the baseline, anchored at 10k and 100k
as a cost cutoff, not a capacity result), and **Flow** (Meta's checker, built for exactly
...
The corpus is layered with **fixed depth**: modules sit in 100 layers, each importing up to three
...
~5,000 modules** (`RangeError: Maximum call stack size exceeded`, reproduced and recorded
...
every sample plus no saved state: the fresh CI runner), **full** (caches warm, no saved
...
red paths: **fullWithLeafErrors** (the same full check over a corpus carrying three type
...
tsbuildinfo); the rows answer different questions and are labeled as such in the JSON. The ts daemons at these scales are
measured below ("The Daemons at a Million Files"); the monorepo editor loop on one
app's closure is in [LIMITS.md](LIMITS.md#editor-and-language-server).

Every timed number sits behind gates: a seeded type error must turn each checker red,
and the program must count exactly N source files (`--listFiles` / `flow ls`), so a
...
row it ran (the tsc anchor stopped at its cost cutoff; tsc did not fail).
...
only to 69ms at 1M (~13% superlinear drift). Flow completes the full sweep at +51% of
tsgo at 10k and +32% at 1M (90.6s vs 68.7s), while the tsc anchor at 100k (66.8s vs
...
### Checking Programs with Type Errors
...
94.8s at 1M, Flow's server init 90.5s (its init is itself a full check). The tsc anchor adds
...
floor dominates, not the edit; its incremental engine is also the component that
stack-overflows on depth-growing graphs (the chainShapeNote above).

### The Daemons at a Million Files

The rows above measure CLI mechanics; the ts mechanic that matches Flow's server is a
persistent daemon. `scripts/lsp-scale-bench.mjs` (→ `bench/lsp-scale-bench.json`) races
**tsgo `--lsp`** against
...
a seeded error must pull TS2322, the restore must pull clean; both are timed):
...
(clear) with definition at 248ms and hover at 1ms; peak RSS is 66.1GB, 23% above the
...
`tsgo --watch` re-checks one edit in 21.9s at 1M (82.0s first build at 75.3GB, the
...
### Codegen in Front of the Checkers: Relay at 10,000 Components
...
artifact per query in **either dialect** (`language: "typescript"` or `"flow"`).
`scripts/relay-codegen-bench.mjs` scaffolds the same 10,000-component tree in both
dialects against a shared
...
**The codegen stage dominates the pipeline**: relay-compiler costs ~4s
to the checkers' 0.7–1.6s, so the checker is not the bottleneck behind a codegen step at
this scale, and Relay's no-change rerun costs the same as cold (a one-shot invocation
re-extracts and re-validates every document even with all artifacts present). There is also a
...
### Released 0.321's Server Crash-Wedge
...
starves, and the master parks every thread against a live RPC socket. The symptom is
...
`bench/flow-wedge-retest.json`): the recheck path was rebuilt.

At a million modules the mechanics sort by persona:

- **The fresh CI runner** pays the cold first check ("Full Check" table): 89.8s on tsgo, 90.8s on flow main.
- **The gate owner** pays the warm whole-program run, red or green, and budgets the RAM ("Memory" table): tsgo is faster, Flow runs on a third of the memory.
- **The developer on save** has two interactive mechanics ("The Developer Loops" and "The Daemons"): Flow's server (324ms per edit, the fastest measured) and tsgo's LSP; `--watch` and the CLI incremental are not save loops at this scale.
